...
from main.calculations.ghi_to_gii import gii_device_id
...
def compute_and_persist_inverter_expected_power(
    *,
    ...
    # For now, require temperature_devices to be in new-format with metric name,
    # so we can read the correct metric time series deterministically.
    if not temp_pick or not temp_pick[0] or not temp_pick[1]:
        raise ValueError(
            f"Inverter '{inverter_id}' weather_device_config.temperature_devices must be configured "
            f"as objects with device_id and metric (new format)."
        )

    temp_device_id, temp_metric = temp_pick
    wind_device_id, wind_metric = (wind_pick or ("", ""))

    temp_by_ts = _load_timeseries(temp_device_id, temp_metric, start_ts, end_ts)
    wind_by_ts = _load_timeseries(wind_device_id, wind_metric, start_ts, end_ts) if wind_device_id and wind_metric else {}
    ...
    # Delete existing expected_power in the window (idempotent overwrite)
    ...
    # SDM path requires temperature; PR path can run without it (no temp correction).
    for ts in base_ts:
        ambient_temp = temp_by_ts.get(ts)
        if not use_pvsyst_pr and ambient_temp is None:
        ...
        rows_for_db.append((ts, {"expected_power": float(pac)}))
