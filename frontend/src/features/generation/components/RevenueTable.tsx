/**
 * Revenue Table Component - Displays hierarchical revenue data
 */
...
// CSV Export Function for Revenue Table
// eslint-disable-next-line react-refresh/only-export-components
export const exportRevenueToCSV = (rows: HierarchicalRow[]) => {
  ...
  rows.forEach((row) => {
    ...
    const actVsIcPct = row.ic > 0 ? ((row.act || 0) / row.ic) * 100 : 0;
    const forecastVsIcPct = row.ic > 0 ? ((row.forecast || 0) / row.ic) * 100 : 0;
    ...
    const rowData = [
      ...
      Math.round(row.act || 0).toString(),
      actVsIcPct > 0 ? actVsIcPct.toFixed(1) : '',
      Math.round(row.forecast || 0).toString(),
...
export function RevenueTable({ rows, onToggleExpand, expandedRows, renderExportButton = true }: RevenueTableProps) {
  ...
  const renderRow = (row: HierarchicalRow) => {
    ...
    return (
      ...
      >
        ...
        <td className="px-2 py-1.5 text-center" style={{ color: tableRowText, fontSize: `${bodyFontSize}px`, borderRight: `1px solid ${tableColumnBorder}` }}>${formatNumber(row.act || 0)}</td>
        <td className="px-2 py-1.5 text-center" style={{ borderRight: `1px solid ${tableColumnBorder}` }}>
          {renderProgressBar(row.act || 0, row.ic, row.act && row.act >= row.ic ? '#22c55e' : '#f97316')}
        </td>
        <td className="px-2 py-1.5 text-center" style={{ color: tableRowText, fontSize: `${bodyFontSize}px`, borderRight: `1px solid ${tableColumnBorder}` }}>${formatNumber(row.forecast || 0)}</td>
        <td className="px-2 py-1.5 text-center">
          {renderProgressBar(row.forecast || 0, row.ic, '#3b82f6')}
        </td>
      </tr>
    );
  };
  ...
  const exportButton = (
    <button
      onClick={() => exportRevenueToCSV(rows)}
      ...
      >
        ...
        >
          <tr>
            ...
            <th className="whitespace-nowrap px-2 py-1.5 text-center font-semibold tracking-wide" style={{ color: tableHeaderText, fontSize: `${headerFontSize}px`, borderRight: `1px solid ${tableColumnBorder}` }}>Actual Rev ($)</th>
            <th className="whitespace-nowrap px-2 py-1.5 text-center font-semibold tracking-wide" style={{ color: tableHeaderText, fontSize: `${headerFontSize}px`, borderRight: `1px solid ${tableColumnBorder}` }}>
              Act vs IC
            </th>
            <th className="whitespace-nowrap px-2 py-1.5 text-center font-semibold tracking-wide" style={{ color: tableHeaderText, fontSize: `${headerFontSize}px`, borderRight: `1px solid ${tableColumnBorder}` }}>
