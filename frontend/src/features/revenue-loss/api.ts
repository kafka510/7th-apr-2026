import type { RevenueLossData } from './types';
...
function normalizeRevenueLossData(data: unknown[]): RevenueLossData[] {
  return data.map((row) => {
    ...
    return cleaned as unknown as RevenueLossData;
  });
}

export async function fetchRevenueLossData(signal?: AbortSignal): Promise<RevenueLossData[]> {
  try {
    const response = await fetchJson<unknown[]>(YIELD_DATA_ENDPOINT, { signal });
    return normalizeRevenueLossData(response);
  } catch (error) {
