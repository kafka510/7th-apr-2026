...
import type { LossCategory } from '../types';
import { useTheme } from '../../../contexts/ThemeContext';

type LossCategoriesAdminProps = {
  isSuperuser: boolean;
};

export const LossCategoriesAdmin = ({ isSuperuser }: LossCategoriesAdminProps) => {
  ...
  const infoBg = theme === 'dark' ? 'rgba(59, 130, 246, 0.2)' : 'rgba(219, 234, 254, 1)';
  const infoBorder = theme === 'dark' ? 'rgba(59, 130, 246, 0.5)' : 'rgba(191, 219, 254, 1)';
  ...
  const loadCategories = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetchLossCategories();
  ...
  const handleEdit = (category: LossCategory) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateLossCategory(editingId, formData);
      } else {
        await createLossCategory(formData);
      }
      setShowForm(false);
      setEditingId(null);
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm('Are you sure you want to delete this category? Tickets using this category will have their loss category cleared.')) {
      return;
    }

    try {
      const result = await deleteLossCategory(id);
      await loadCategories();
      setError(null);
  ...
  return (
      ...
      >
      <div 
        className="border-b bg-gradient-to-r from-cyan-600 to-cyan-700 px-6 py-4"
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-white">Loss Categories</h2>
          </div>
          <button
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  style={{
                    backgroundColor: theme === 'dark' ? 'rgba(6, 182, 212, 0.8)' : '#0891b2',
                  }}
                  onMouseEnter={(e) => {
                    if (!submitting) {
                      e.currentTarget.style.backgroundColor = theme === 'dark' ? 'rgba(6, 182, 212, 1)' : '#0e7490';
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (!submitting) {
                      e.currentTarget.style.backgroundColor = theme === 'dark' ? 'rgba(6, 182, 212, 0.8)' : '#0891b2';
                    }
                  }}
                >
